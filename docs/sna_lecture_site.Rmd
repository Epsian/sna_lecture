...
| Ameiko Kaijitsu | Alyx Vance    |
| Paul Atreides | Alyx Vance    |
| Levi Ackerman | Alyx Vance    |
...
| Jesse Faden | Spike Spiegel  |
| Ramona Flowers | Spike Spiegel  |
| Jesse Faden | Edward Elric    |

```{r include=FALSE}
# create this network silently for easy matrix conversion
toy_edgelist = data.frame(from = c("Jesse Faden", "Levi Ackerman", "Samantha Carter", "Ameiko Kaijitsu", "Paul Atreides", "Levi Ackerman", "Alyx Vance", "Jesse Faden", "Ramona Flowers", "Gordon Freeman", "Alyx Vance", "Ramona Flowers", "Jesse Faden", "Ramona Flowers", "Jesse Faden"), to = c("Samantha Carter", "Samantha Carter", "Alyx Vance", "Alyx Vance", "Alyx Vance", "Alyx Vance", "Ramona Flowers", "Ameiko Kaijitsu", "Ameiko Kaijitsu", "Paul Atreides", "Gordon Freeman", "Levi Ackerman", "Spike Spiegel", "Spike Spiegel", "Edward Elric"), stringsAsFactors = FALSE)
```
...
We'll use the `network` function to create our network object.

```{r}
toy_network = network(toy_edgelist)
```
...
There we are. The default plotting in `statnet` is ugly. For the sake of our eyes, and for exploring some of the measure we create, we'll use the `visNetwork` package to visualize out networks. It will make the code a bit more cumbersome, but it will be worth it. Let's try that again with `visNetwork`. We'll give the `visNetwork` function our edgelist and attributes dataframe.
...
```{r echo=FALSE}
.dir_node$value = c(3, 6, 4, 4, 3, 2, 2, 3, 2, 1)^3
...
The example below plots a network, and shows each node's average geodesic distance below the node. The red node would need to travel through the whole network to reach the nodes on the opposite side and has the largest average geodesic distance. Meanwhile, the blue node has the smallest average geodesic distance because it is located near the middle of the network.

```{r Geodesic, echo=TRUE, message=FALSE, warning=FALSE}
# convert to network, find geodesics
vis_net = network::network(.dir_edge[,c("from", "to")], directed = FALSE)
vis_net%v%"mean.distance" = (colSums(geodist(vis_net)$gdist))/(nrow(as.sociomatrix(vis_net)) - 1)

# default grey color
.dir_node$color = c("grey")

# add label as geodesic distance
.dir_node$label = round(vis_net%v%"mean.distance", 3)

# replace min with blue, max with red, add labels
.dir_node$color[which(vis_net%v%"mean.distance" == max(vis_net%v%"mean.distance"))] = "red"
.dir_node$color[which(vis_net%v%"mean.distance" == min(vis_net%v%"mean.distance"))] = "blue"

# make sure edges are grey
.dir_edge$color = "grey"

# plot
visNetwork(.dir_node[,-2], .dir_edge[,c("to", "from", "color")], main = "Geodesic Example") %>% visInteraction(zoomView = FALSE)
```
