...
Each network also typically has an attributes table, which looks just like typical tabular data, with each row belonging to a specific node in our network. For the examples above, our attribute table might look like the following:
...
Before we move on we should take a moment to talk about some the the caveats when suing network data. Network analysis is very vulnerable to missing data. A simple way to understand why is to make a small adjustment to our network.

I've highlighted one node in green. This node is structurally vital to the network we see now.

```{r net_data_vulnerability}
...
```{r net_data_vulnerability_missing}
# plot
...
**K-Core** help find highly connected segments in a network. These clusters are a rudimentary measure of community in a network, though much more nuanced tools exist. Nodes are part of a k-core if all others in the group are connected with *k* other nodes. So for example a group would be a 3-core if every node was connected to at least 3 other nodes in a cluster.

```{r}
# compute kcore and save to "kcore"
toy_network%v%"kcore" = kcores(toy_network, mode = "graph")

# change labels to kcore
toy_attributes$label = toy_network%v%"kcore"

# reset all nodes to grey
toy_attributes$color = c("grey")

# plot node colors according to kcore
toy_attributes$color = colors(distinct = TRUE)[toy_network%v%"kcore" + 50]

# plot
visNetwork(toy_attributes, toy_edgelist, main = "K-core Example") %>% visInteraction(zoomView = FALSE)
```

# Network Tools

There are several ways to interact with network data in R. Thus far we have been using a combination of `statnet` for analysis and `visNetwork` for visualization. Next we'll dig deeper into these tools and explore other options.

## statnet

`statnet` is one of the two largest network packages in R. 
