...
When you are done, look in your working directory for `YOUR_NAME_network_homework.answers`. This is what you will upload to Canvas. Please replace "YOUR_NAME" with your name. If you need help finding your working directory, run `getwd()` in the console.
...
## Problem Set 1 - Bird's Eye View

These problems will all relate to a network as a whole. One of the main draws of network analysis is its ability to get a "bird's eye view" of a system that normally we as individuals only see a small part of. 
...
```{r directed}
question("\nQ1. Is this network directed or un-directed?",
...
```{r density}
question_text("\nQ2. What is the density of this network?",
...
```{r net_text}
textAreaInput("net_text", "Q3. Why do you think?")
actionButton("save_button_net_text", label = "Save")
verbatimTextOutput("net_text_saved")
```

```{r net_text_back, context="server"}
net_text_saved_text = eventReactive(input$save_button_net_text, {
  cat("\nQ3. Why do you think?\n", input$net_text, file = .answers_log, append = TRUE)
  paste0("Saved Answer: ", input$net_text)
  })

output$net_text_saved = renderText({net_text_saved_text()})
```

## Problem Set 2 - Individual Measures
...
This section will go over some network concepts and vocabulary. Be sure to hit the "Continue" button after you finish each section of questions!
